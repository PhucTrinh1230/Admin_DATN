  ...
  const deleteRating = async (id) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:8080/api/rating/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const editRating = async (e,id) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8080/api/rating/${id}`, raTing,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
    ...
    >
      ...
      <div className="row g-4">
        ...
        <button
          ...
          onClick={(e) =>editRating(e, raTing.id)}
        >
          Cập nhật
        </button>
        ...
        <tbody>
          {raTings.map((raTing, index) => (
            <tr key={index}>
              ...
              <td>{raTing.account.username}</td>

              {/* Cột ID ẩn đi */}
              <td style={{ display: "none" }}>{raTing.id}</td>

              <td>
                <button
                  className="btn btn-warning mx-3"
                  onClick={() => viewsRating(raTing.id)}
                >
                  Chi tiết
                </button>
                <button
